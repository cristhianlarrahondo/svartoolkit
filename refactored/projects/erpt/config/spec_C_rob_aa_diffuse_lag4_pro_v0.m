%spec_C_rob_aa_diffuse_lag4_pro_v0  ERPT Ejercicio C -- sistema PRODUCER
...
%   pro_inf (inflacion al productor) como unica variable de precio.
%
%   Variables endogenas (Cfg.VARS), orden posicional:
%     1. ner      -- Nominal Exchange Rate
%     2. pro_inf  -- Producer Inflation
...
%   A diferencia del sistema de imports, aqui NO se aplica la asimetria
%   ner⊥Ofe=0: pro_inf recibe signo bajo Dem (pro+) y queda LIBRE bajo Ofe
%   (producer prices con respuesta ambigua al choque de oferta, por diseno
%   -- DC-4). El choque de Oferta se identifica por ea(+) y tot=0.
%
...
Cfg.SPEC_NAME = 'spec_C_rob_aa_diffuse_lag4_pro_v0';

% -- DATOS --------------------------------------------------------------
Cfg.DATA_FILE = fullfile(ex_dir, 'data', 'data_erpt_aa.xlsx');
Cfg.VARS      = {'ner', 'pro_inf', 'ea', 'ir', 'tot'};
...
% Indices: 1=ner, 2=pro_inf, 3=ea, 4=ir, 5=tot.
...
% -- Shock 2: Dem : pro(+), ea(+), ir(+) | tot=0  (1 cero) --------------
...
% -- Shock 3: Ofe : ea(+) | tot=0  (1 cero; pro_inf LIBRE bajo Ofe) -----
