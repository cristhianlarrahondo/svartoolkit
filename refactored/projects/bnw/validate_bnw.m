%VALIDATE_BNW  Script de verificación — Chat 16: Loader extendido + projects/bnw.
%
%   Tipo R (modifica load_data.m — requiere regresión numérica exacta).
%
%   SECCIÓN A — Composición de specs (base + overrides), projects/bnw/
...
%     A4: Cfg.OUTPUT_DIR de ambos apunta a projects/bnw/output (nunca a
%         refactored/output)
%
...
%
%   SECCIÓN E — Proyecto projects/bnw/ (requiere data_bnw.xlsx copiado a
%   projects/bnw/data/; si no está presente, E2–E5 reportan FALLA con
%   instrucciones claras, sin abortar el resto del script)
%     E1: archivo de datos presente en projects/bnw/data/data_bnw.xlsx
...
%     E3: PFA en projects/bnw (nd=1e4, rng(0)) reproduce
%         Ltilde(end,end,end) == -0.2326865051
%     E4: IS en projects/bnw (nd=3e4, rng(0)) reproduce
%         Ltilde(end,end,end,end) == 0.2041864191
%     E5: plot_irfs/export_results escriben en projects/bnw/output/, y NO
...
fprintf('\n');
fprintf('================================================================\n');
fprintf(' VALIDATE_BNW — Chat 16: Loader extendido + projects/bnw\n');
fprintf('================================================================\n\n');

%% ── Rutas ────────────────────────────────────────────────────────────────
% validate_bnw.m vive en: refactored/projects/bnw/
val_root = fileparts(mfilename('fullpath'));      % .../projects/bnw/
...
%% ── Checker de rutas prohibidas (aplica a src/ y projects/bnw/) ─────────
DOTDOT  = '\.\.[/\\]';
ruta_ok = true;
...
%% SECCIÓN A — Composición de specs (base + overrides), projects/bnw/
%% ═══════════════════════════════════════════════════════════════════════
fprintf('SECCIÓN A — Composición de specs (projects/bnw)\n');
fprintf('-------------------------------------------------\n');
...
[n_pass, n_fail] = rpt(n_pass, n_fail, ...
    'A4: Cfg.OUTPUT_DIR apunta a projects/bnw/output (no a refactored/output)', ok, detail);
...
%% ═══════════════════════════════════════════════════════════════════════
%% SECCIÓN E — Proyecto projects/bnw/
%% ═══════════════════════════════════════════════════════════════════════
fprintf('SECCIÓN E — Proyecto projects/bnw/\n');
fprintf('-------------------------------------\n');

n_fail_before_E = n_fail;   % para distinguir fallas de A-D de las de E (archivo faltante)
data_present = isfile(ex_data);
[n_pass, n_fail] = rpt(n_pass, n_fail, 'E1: data_bnw.xlsx presente en projects/bnw/data/', data_present, ...
...
[n_pass, n_fail] = rpt(n_pass, n_fail, ...
    'E3: PFA projects/bnw (nd=1e4, rng(0)) reproduce Ltilde(end,end,end)', ok, detail);
...
[n_pass, n_fail] = rpt(n_pass, n_fail, ...
    'E4: IS projects/bnw (nd=3e4, rng(0)) reproduce Ltilde(end,end,end,end)', ok, detail);
...
[n_pass, n_fail] = rpt(n_pass, n_fail, ...
    'E5: outputs en projects/bnw/output/, nada nuevo en refactored/output/', ok, detail);
...
elseif ~data_present && n_fail_before_E == 0
    ...
    fprintf('            (falta copiar data_bnw.xlsx a projects/bnw/data/)\n');
else
