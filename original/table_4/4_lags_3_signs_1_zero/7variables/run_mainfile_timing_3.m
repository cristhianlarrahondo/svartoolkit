...
% 6: Investment
% 7: Output
data = importdata('data/data.csv'); % import Beaudry, Nam and Wang (2011) data
num  = data.data(:,3:9)*100;            % get variables to be used in SVAR
...
horizons  = 0;               % horizons to restrict
NS        = 1;               % number of objects in F(A_{0},A_{+}) to which we impose sign and zero restrictios: F(A_{0},A_{+})=[L_{0}]
...
if strcmp(conjugate,'irfs')==1
    ...
    r_irfs          = @(x)IRFRestrictions(x,iw_info); 
end
