...
% 1: Real GDP               (Chain-Weighted Billions  of 2009)
% 2: GDP Deflator           (2009=100)
% 3: PCOM                   (Dow Jones Spot Average (Symbol _DJSD) Commodity Price Index, commercially available from Global Financial Data)
% 4: M2                     (Billions of Dollars)
% 5: Federal Funds Rate     (annualized FFR)
% 6: Unemployment Rate      (annualized)
% 7: Macroeconomic Uncertainty  Index (Kyle Jurado, Sydney C. Ludvigson, Serena NG (2015))
num = xlsread('data/data.xlsx');

%% data
% all variables are in log times 100 except for the federal funds rate that enters the SVAR in annualized percentages
num  = [100*log(num(:,1)) 100*log(num(:,2)) 100*log(num(:,3)) 100*log(num(:,4)) 100*num(:,5) 100*num(:,6) 100*log(num(:,7))];


%% model setup
nlag      = 12;                  % number of lags
nvar      = 7;                   % number of endogenous variables
    ...
    %    count=count+1;
        ...
        switch conjugate
            ...
            case 'irfs'
                ...
                storevephi(record,1)   = LogVolumeElementOneSidedND(fo,structpara)   + LogVolumeElementOneSidedND(fo_str2irfs_inv,irfpara);
                storevegphiZ(record,1) = LogVolumeElementOneSidedND(fs,structpara,r) + LogVolumeElementOneSidedND(fo_str2irfs_inv,irfpara,r_irfs); 
...
save('results/timing_three.mat','tElapsed','count','nd','ne')
message = 'Done.';
disp(message);
